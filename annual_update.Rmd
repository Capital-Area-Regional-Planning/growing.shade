...
The data set that is released the latest is USDA’s NASS CDL, which is required to separated canopy from cropland. The most recent 2022 layer was released on January 30, 2023, so it is expected that future releases will also be in late January. 
...
wide_ctu_crosswalk <- ctu_crosswalk %>%
  ...
  save(ctu_crosswalk, file = paste0(here::here(), "/data/ctu_crosswalk.rda"))
  save(nhood_crosswalk, file = paste0(here::here(), "/data/nhood_crosswalk.rda"))
...
#TODO the error calculation code is in the KPI canopy script right now
  bg_canopy <-
    read_csv(paste0(here::here(), "/data-raw/TreeAcres_bg_year2023.csv")) %>%
    arrange(GEOID) %>%
    remove_lake_bgs("GEOID") %>%
    left_join(read_csv(
      paste0(
        here::here(),
        "/data-raw/canopy_error_bg.csv")),
        join_by(GEOID == GEOID)
      ) %>% 
    mutate(`1` = `1` + error) %>% 
    select(-error)
      
  ctu_list_raw <- read_csv(paste0(here::here(), "/data-raw/TreeAcres_ctu_year2023.csv")) %>% 
  arrange(GEO_NAME) %>% 
    left_join(read_csv(
      paste0(
        here::here(),
        "/data-raw/canopy_error_ctu.csv")),
        join_by(GEO_NAME == GEO_NAME)
      ) %>% 
    mutate(`1` = `1` + error) %>% 
    select(-error)
  
  nhood_list <- read_csv(paste0(here::here(), "/data-raw/TreeAcres_nhood_year2023.csv")) %>% 
  arrange(GEO_NAME) %>% 
    left_join(read_csv(
      paste0(
        here::here(),
        "/data-raw/canopy_error_nhood.csv")),
        join_by(GEO_NAME == GEO_NAME)
      ) %>% 
    mutate(`1` = `1` + error) %>% 
    select(-error)
