...
library(RSocrata)
library(here)
#Added the terra library for processing for Dane County
#The same code could likely be written with sf, but I'm personally more familiar with terra
#library(terra)
...
####
state <- c("WI")
county <- c("Dane") # either type in specific county names, or for all counties within a state assign `county <- NULL`
...
*See 02_geographies.Rmd for processing for Dane County*
...
*See 03_remotesensing.Rmd for processing for Dane County*
...
*The 2023 data was available, but I had issues automating the downloaded. I downloaded it manually from the link above, saved it in the data-raw folder, and commented out the automation portion of the chunk below*
...
if (regional_specific_data == FALSE) {
  ...
  #details about this table are in 03_remotesensing - Liz
...
*The original code used data specific to the Twin Cities region, so I rewrote this chunk.*
...
holc_data <- data.frame(matrix(nrow = 353, ncol = 2))
...
ui_params <- tribble(
  ...
  "nhoodselected", "Capitol Neighborhoods", NA_real_,
...
*I initially had trouble with incompatibility between the councilR library and my version of R, but was able to download by running remotes::install_github("Metropolitan-Council/councilR")*
